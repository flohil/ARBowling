...
namespace Vuforia
{
    ...
    public class DefaultInitializationErrorHandler : MonoBehaviour
    {
        ...
        void OnGUI()
        {
            ...
            if (mErrorOccurred)
                UnityEngine.GUI.Window(0, new Rect(0, 0, Screen.width, Screen.height),
                                       this.DrawWindowContent, WINDOW_TITLE);
        }
        ...
        private void DrawWindowContent(int id)
        {
            ...
            // window border.
            UnityEngine.GUI.Label(new Rect(10, 25, Screen.width - 20, Screen.height - 95),
                        mErrorText);

            // Create centered button with 50/50 size and 10 pixel distance from
            // other controls and window border.
            if (UnityEngine.GUI.Button(new Rect(Screen.width / 2 - 75, Screen.height - 60, 150, 50), "Close"))
            {
#if UNITY_EDITOR
